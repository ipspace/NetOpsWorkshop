...
Each subdirectory contains a typical build script (_build.sh_) and a run script (_run.sh_) that you can use to start the container as a non-root user.

As CentOS has been EOL for years, you can only build a Ubuntu-based container.

Please feel free to extend the container to fit your needs:

* To packages, modify _requirements-yum.txt_ or _requirements-apt.txt_.
